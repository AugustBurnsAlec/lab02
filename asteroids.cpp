...
public:
	Game() {
		...
		for (int j=0; j<10; j++) {
			...
			a->color[0] = ((float)rand()/ (RAND_MAX)) ;
			a->color[1] = ((float)rand()/ (RAND_MAX)) ;
			a->color[2] = ((float)rand()/ (RAND_MAX)) ;
		...
		if (e->xbutton.button==1) {
			...
			if (ts > 0.1) {
				...
				if (g.nbullets < MAX_BULLETS) {
					...
					b->pos[0] += xdir*20.0f;
					b->pos[1] += ydir*20.0f;
					b->vel[0] += xdir*60.0f + rnd()*0.1;
					b->vel[1] += ydir*60.0f + rnd()*0.1;
					b->color[0] = rnd();
					b->color[1] = rnd();
					b->color[2] = rnd();
					++g.nbullets;
	...
	if (gl.keys[XK_space]) {
		...
		if (ts > 0.1) {
			...
			if (g.nbullets < MAX_BULLETS) {
				...
				b->pos[0] += xdir*20.0f;
				b->pos[1] += ydir*20.0f;
				b->vel[0] += xdir*BULL_SPEED + rnd()*BULL_VAR;
				b->vel[1] += ydir*BULL_SPEED + rnd()*BULL_VAR;
				b->color[0] = rnd();
				b->color[1] = rnd();
				b->color[2] = rnd();
				g.nbullets++;
...
void render()
{
	...
	ggprint8b(&r, 16, 0x00ffff00, "n asteroids destroyed: %i", g.deleted);
	//
	//-------------
	//Draw the ship
